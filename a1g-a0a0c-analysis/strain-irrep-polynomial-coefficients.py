...
for k, lists in enumerate(list_of_lists):
    ...
    for i,item in enumerate(number_of_included_coefficients):
        ...
        if k == 0:
            ...
            plt.plot([0,0.2,0.4,0.6,0.8,1], j1xy_matrix[0,:],color='black')
            plt.plot([0,0.2,0.4,0.6,0.8,1], j1xy_matrix[1,:],color='black')
            plt.plot([0,0.2,0.4,0.6,0.8,1], j1xy_matrix[2,:],color='black')
            plt.plot([0,0.2,0.4,0.6,0.8,1], j1xy_matrix[3,:],color='black')
            plt.plot([0,0.2,0.4,0.6,0.8,1], j1xy_matrix[4,:],color='black')
            plt.plot([0,0.2,0.4,0.6,0.8,1], j1xy_matrix[5,:],color='black')

            #plt.plot([0,0.2,0.4,0.6,0.8,1],estimate_matrix[0,:])
            #plt.plot([0,0.2,0.4,0.6,0.8,1],estimate_matrix[1,:])

            #plt.plot([0,0.2,0.4,0.6,0.8,1], j1xy_matrix[:,0])
            #plt.plot([0,0.2,0.4,0.6,0.8,1],estimate_matrix[:,0])
            plt.legend(['J1xy $\phi$=0 estimate','J1xy actual valuesyhhh'])
